...
#pragma omp parallel for
    ...
    for (int i = 0; i < NUM_TRAIN; i++)    
    {
        ...
        string impath=format(_S(_voc.imgPathW), _S(_voc.trainSet[i]));
        if ( access(impath.c_str(),F_OK)  != 0 )
        {
			cerr<<"File [ "<<impath<<" ] not found !  In generateTrianData()"<<endl;
			continue;
		}
...
Mat Objectness::getFeature(CMat &img3u, const Vec4i &bb)
{
 ...
 //   cout<<"reg="<<x<<","<<y<<","<<bb[2]-x<<","<<bb[3]-y<<endl;
...
void Objectness::trainStageI()
{
    ...
    cout<<"_modelName="<<_modelName<<endl;
    CV_Assert(matRead(_modelName + ".xP", xP1f) && matRead(_modelName + ".xN", xN1f));
